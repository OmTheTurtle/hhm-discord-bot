...
module.exports = async (userMessage) => {
    if(!userMessage.author.bot) {
        if (userMessage.content.startsWith(DEFAULT_PREFIX))
            ...
            await tryToExecuteCommand(userMessage, DEVELOPER_PREFIX, 'developer-commands');
        else
            await countMessages(userMessage);
    }
};
...
function logToDeveloperChannel(message, error) {
    const developerChannel = message.channel.guild.channels.cache.get(DEVELOPER_CHANNEL_ID);
    developerChannel.send(`**An error occurred:**`);
    developerChannel.send(`Message info:\`\`\`json\n${JSON.stringify(message, null, "\t")}\`\`\``);
