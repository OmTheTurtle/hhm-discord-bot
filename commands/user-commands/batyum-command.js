...
module.exports = async (message) => {
  ...
  if (!user)
    return message.channel.send(
      `${message.author.toString()} te nem létezel az adatbázisban, vagy <@${DEVELOPER_GUNTHER_USER_ID}> megint elbaszott valamit.`
    )

  const allCoins = await User.sum("coin", { where: { coin: { [Op.gt]: 0 } } })
  message.channel.send(
    `A batyudban ${user.coin} tallér van, ami a szerver vagyonának ${(
      (user.coin / allCoins) *
      100
    ).toFixed(2)}%-a.`
  )
